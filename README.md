...
- **桌面小工具** — Android Widget 直接顯示常用條碼，支援多卡切換
- **最近門市提示** — 無符合時顯示 1km 內最近門市與距離
- **加密儲存** — Hive 加密資料庫 + Android KeyStore
- **多種輸入** — 相機掃描 / 截圖辨識 / 手動輸入
- **38 品牌資料庫** — 內建台灣常見品牌 + 14,000+ 門市 GPS 座標（OSM）
...
| 框架 | Flutter 3.x |
| 本地 DB | Hive（加密） |
| 桌面小工具 | home_widget + Kotlin RemoteViews |
...
├── data/
│   └── store_locations.json  # 門市 GPS 座標資料庫（14,000+ 筆）
├── models/
