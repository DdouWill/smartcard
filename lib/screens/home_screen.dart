  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(onPressed: _controller.runLocationDetection, icon: const Icon(Icons.my_location)),
          IconButton(onPressed: () => AppRouter.pushSettings(context), icon: const Icon(Icons.settings)),
        ],
      ),
          ...
          return Column(
            children: [
              LocationStatusCard(
                ...
                onCardTap: (card) => AppRouter.pushCardDetail(context, card: card),
              ),
              Expanded(
                child: _controller.cards.isEmpty
                    ? const Center(child: Text('尚未新增會員卡'))
                    : ReorderableListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return CardWidget(
                            ...
                            card: card,
                            onTap: () => AppRouter.pushCardDetail(context, card: card),
                            onDelete: () => _deleteCard(card),
                          );
                        },
                        onReorder: (oldIndex, newIndex) {
                          if (newIndex > oldIndex) newIndex--;
                          final ids = _controller.cards.map((c) => c.id).toList();
